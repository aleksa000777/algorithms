...
function solution(X, A) {
   ...
   var count;
    ...
    arr.sort(function(a,b){
        if(a-b>1 || b-a>1){return count=-1;}
    })
    if(A[0]===x && A[1]===undefined){return count=0}
    if(A[0]===x&&A[1]===x){return count =-1}
    if(A.indexOf(x)===-1){return count=-1};
     if(count===undefined){
        A.indexOf(x) ? count=A.indexOf(x)-1 : count=-1;
     }
    return count;
}
